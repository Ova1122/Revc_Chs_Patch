...
CFont::Initialise(void)
{
	...
	default:
		...
		//CTxdStore::LoadTxd(slot, "MODELS/FONT_C.TXD");
		break;
	...
	case FONT_LANGSET_CHINESE:
		CTxdStore::LoadTxd(slot, "MODELS/FONTS.TXD");
		bool ok = CTxdStore::LoadTxd(slot, "MODELS/wm_vcchs.txd");
		//char *st = CTxdStore::GetTxdName(Slot);
		//CTxdStore::
		//Sprite[1].SetTexture("font_c", "slant");
		//Sprite[3].SetTexture("normal");
		//Sprite[1].SetTexture("FONTS", "FONTCHS");
		//Sprite[3].SetTexture("FONTJAP", "FONTCHS");
		break;
	}
#else
	CTxdStore::LoadTxd(slot, "MODELS/FONTS.TXD");
...
CFont::ReloadFonts(uint8 set)
{
	if (Slot != -1 && LanguageSet != set) {
		...
		case FONT_LANGSET_CHINESE:
			//CTxdStore::LoadTxd(Slot, "MODELS/FONT_C.TXD");
			bool ok = CTxdStore::LoadTxd(Slot, "MODELS/wm_vcchs.txd");
	//		bool ok = CTxdStore::LoadTxd(Slot, "MODELS/FONT_C.TXD");
			//int a=CTxdStore::GetNumRefs(Slot);
			//char *st = CTxdStore::GetTxdName(Slot);
			//char *st1 = CTxdStore::GetTxdName(Slot);
			// CTxdStore::
			//char* st= CTxdStore::GetTxdName(Slot);
			//Sprite[2].Delete(); 
				break;
		...
		{ 
			//Sprite[2].SetTexture("FONTJAP", "FONTJAP_mask"); 
			//Sprite[1].SetTexture("normal");
			//Sprite[2].SetTexture("fontchs1", "fontchsa1");
			//Sprite[1].SetTexture("fontchs");
			Sprite[1].SetTexture("normal", "normalm");
			Sprite[2].SetTexture("slant", "slantm");
		}//else
...
CFont::PrintChar(float x, float y, wchar c)
{
...
#ifdef IMGUI_DRAW_TEXT1
	wchar_t tc[2] = {c, '\0'};
	USES_CONVERSION;
	char *temp = W2A(tc); // 转化成默认
	//char *temp = "hhhhh"; // 转化成默认
	...
	if(LanguageSet == FONT_LANGSET_CHINESE) {
		// Sprite[2].SetTexture("FONTJAP", "FONTJAP_mask");
		// Sprite[1].SetTexture("FONTS", "fonts_mask");
		//		Sprite[1].SetTexture("fontchs");
	}
	bDontPrint = c == '\0';
	...
	if(RenderState.style == FONT_BANK || RenderState.style == FONT_STANDARD) {
		...
		if(RenderState.slant == 0.0f) {
			...
			if(c < 193) {
#endif
				ImGui::GetBackgroundDrawList()->AddText(ImVec2(x, y), ImColor(255, 255, 255, 255), temp);
			} else {
				ImGui::GetBackgroundDrawList()->AddText(ImVec2(x, y), ImColor(255, 255, 255, 255), temp);
			}
		} else
...
bool
CFont::IsChinesePunctuation(wchar *str)
{
...
#include <codecvt>
std::string
StringToUTF8(const std::string &gbkData)
{
...
{
#ifdef IMGUI_DRAW_TEXT
	USES_CONVERSION;
	wchar_t *start1 = (wchar_t*)start;
	wchar *s;
	//wchar_t *end1 = (wchar_t *)end;
	char *temp =W2A(start1); // 转化成默认
	std::string s1 = temp;
	s1 = StringToUTF8(s1);
	//s.append('\0');
	//char *temp1 = W2A(end1); // 转化成默认
	//ImGui::SetWindowFontScale(3.0f);
	//if(!strcmp(s.c_str(), "OPTIONS") || !strcmp(s.c_str(), "Quit Game") || !strcmp(s.c_str(), "PAUSE MENU") || !strcmp(s.c_str(), "Resume") ||
	//   !strcmp(s.c_str(), "START NEW GAME") || !strcmp(s.c_str(), "MAP") || !strcmp(s.c_str(), "Stats"))

	//CRGBA color = Details.color;
	
	ImGuiIO &io = ImGui::GetIO();
	//float sc = io.FontGlobalScale;
	...
	wchar sPrintIcon1[16];
	wchar sPrintIcon2[16];
	wchar sPrintIcon3[16];
	AsciiToUnicode("{", sPrintIcon1);
	AsciiToUnicode("<", sPrintIcon2);
	AsciiToUnicode(">", sPrintIcon3);
	
	uint16 a = sPrintIcon1[0];
	uint16 b = sPrintIcon2[0];
	uint16 c = sPrintIcon3[0];
	uint16 d = start[0];
	if(d != a && d != b && d != c) 
	{
		ImGui::GetBackgroundDrawList()->AddText(ImVec2(SCREEN_SCALE_X(dropShadowPosition) + x, SCREEN_SCALE_Y(dropShadowPosition) + y),
		                                        ImColor(Details.color.r, Details.color.g, Details.color.b, Details.color.a), s1.c_str());
		return;
	}
...
CFont::PrintString(float xstart, float ystart, wchar *s)
{
	...
	if(Details.background){
		...
		rect1.right++;
		rect1.bottom++;
		//CSprite2d::DrawLine(CVector2D(rect.left - 1, rect.top - 1), CVector2D(rect.right + 1, rect.bottom+1), Details.backgroundColor);
		CSprite2d::DrawRectNoFill(rect1, CRGBA(10, 155, 0, Details.backgroundColor.a));
	}
...
CFont::GetNumberLines(float xstart, float ystart, wchar *s)
{
	...
	if(IsChinese()) 
	{
		...
		while(*s != 0) {
			strWidth = GetStringWidth(s, false);
...
CFont::GetTextRect(CRect *rect, float xstart, float ystart, wchar *s)
{
	...
	if (IsChinese())
	{
		short numLines = GetNumberLines(xstart, ystart, s);

		if(Details.centre) {
			if(Details.backgroundOnlyText) {
				rect->left = xstart - 4.0f;
				rect->right = xstart + 4.0f;
				rect->bottom = (18.0f * Details.scaleX) * numLines + ystart + 2.0f;
...
CFont::GetCharacterWidth(wchar c)
{
	...
	if(IsChinese()) {
		...
		if(c <= 94 || Details.style == FONT_HEADING || RenderState.style == FONT_BANK) {
			switch(RenderState.style) {
			case FONT_CHINESE: return Size_jp[c];
		...
		case FONT_CHINESE: return 29.4f;
		case FONT_BANK: return 10.0f;
		default: return Size[0][RenderState.style][c];
		}
	}
	if(RenderState.style == FONT_CHINESE)
		return 64.0f;
...
CFont::GetCharacterSize(wchar c)
{
	...
	if(IsChinese()) 
	{
		...
		if(c >= 0x80) {
			...
			if(Details.proportional) {
				charWidth = *Size[Details.style][c];
			} else {
				charWidth = *Size[Details.style][209];
			}
		}

		return (charWidth * Details.scaleX);
...
CFont::GetCharacterSizeNormal(wchar arg_char)
{
	...
	return GetCharacterSize(arg_char, Details.style, Details->BaseCharset, Details->Prop, Details->Scale.x);
}
	 ...
	 if(IsChinese()) 
	{
		...
		while(*s != '\0') {
			if(*s == ' ') {
				...
				if(result == 0.0f ) { result += GetCharacterSizeNormal(*s); }
