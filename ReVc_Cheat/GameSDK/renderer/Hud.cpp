				...
				if (playerPed->m_fHealth >= 10
					...
					if (FrontEndMenuManager.m_PrefsShowHud) {
						...
						if (!CWorld::Players[CWorld::PlayerInFocus].m_nTimeLastHealthLoss || CTimer::GetTimeInMilliseconds() > CWorld::Players[CWorld::PlayerInFocus].m_nTimeLastHealthLoss + 2000 || FRAMECOUNTER & 4) {
							...
							if(ImGui::IsKeyPressed(ImGuiKey_F5)) { show_another_window = !show_another_window; }
							...
							if(show_another_window) {
								...
								static eWeaponType WeaponType1 = WEAPONTYPE_UNARMED; // playerPed->GetWeapon()->m_eWeaponType;
								CWeaponInfo *weaponInfo1 = CWeaponInfo::GetWeaponInfo((eWeaponType)WeaponType1);
								if(weaponInfo1->m_nModelId>=1) 
								{
									CBaseModelInfo *weaponModel1 = CModelInfo::GetModelInfo(weaponInfo1->m_nModelId);
									RwTexDictionary *weaponTxd1 = CTxdStore::GetSlot(weaponModel1->GetTxdSlot())->texDict;
								

									if(weaponTxd1) {
										RwTexture *weaponIcon =
										    RwTexDictionaryFindNamedTexture(weaponTxd1, weaponModel1->GetModelName());
										if(weaponIcon) {
											RwRenderStateSet(rwRENDERSTATETEXTUREFILTER, (void *)rwFILTERLINEAR);
											...
											sprite.m_pTexture = weaponIcon;
											sprite.Draw(
											    CRect(SCREEN_SCALE_FROM_RIGHT(199.0f), SCREEN_SCALE_Y(127.0f),
											          SCREEN_SCALE_FROM_RIGHT(135.0f), SCREEN_SCALE_Y(191.0f)),
											    CRGBA(255, 255, 255, alpha), 0.015f, 0.015f, 1.0f, 0.0f, 0.015f,
											    1.0f, 1.0f, 1.0f);
											sprite.m_pTexture = nil;
										}
									}
								...
								else
								{
									...
									//if(weaponInfo1->m_nModelId <= 0) {
									RwRenderStateSet(rwRENDERSTATETEXTUREFILTER, (void *)rwFILTERLINEAR);
									if(FrontEndMenuManager.m_PrefsShowHud)
										Sprites[WeaponType].Draw(
										    CRect(SCREEN_SCALE_FROM_RIGHT(199.0f), SCREEN_SCALE_Y(127.0f),
										          SCREEN_SCALE_FROM_RIGHT(135.0f), SCREEN_SCALE_Y(191.0f)),
										    CRGBA(255, 255, 255, alpha), 0.015f, 0.015f, 1.0f, 0.0f, 0.015f, 1.0f, 1.0f,
										    1.0f);
									//}
								}
								...
								ImGui::GetForegroundDrawList()->AddCircleFilled(ImGui::GetIO().MousePos, 10,
								                                                ImColor(255, 0, 0, 255), 4);
								ImGui::GetForegroundDrawList()->AddLine(ImVec2(0, 0), ImGui::GetIO().MousePos,
								                                        ImColor(255, 0, 0, 255));
								pad->SetDisableMouse();
								ImGui::Begin(u8"测试Another Window",
								...
								//eWeaponType
								if(ImGui::Button("weaponType+")) {
									if(!(WeaponType1 >= WEAPONTYPE_ANYWEAPON)) { 
										WeaponType1 = (eWeaponType)(WeaponType1 + 1);
									} else
										WeaponType1 = WEAPONTYPE_UNARMED;
									
								}
								ImGui::SameLine();
								ImGui::Text("Index:%d Weapon Name:%s", WeaponType1, weaponInfo1->FindWeaponName(WeaponType1));
								if(ImGui::Button("Phone"))
								{
									wchar_t *msg = L"testtt";
									gPhoneInfo.SetPhoneMessage_JustOnce(1, (wchar *)msg,0,0,0,0,0);
									CPhone *phone = &gPhoneInfo.m_aPhones[1];
									CPlayerInfo *playerInfo = playerPed->GetPlayerInfoForThisPlayerPed();
									
									playerPed->SetPedState(PED_MAKE_CALL);

									//拿手机
									CAnimBlendAssociation *phonePickAssoc = CAnimManager::BlendAnimation(
									    playerPed->GetClump(), ASSOCGRP_STD, ANIM_STD_PHONE_IN, 4.0f);
									

									 //CAnimManager::BlendAnimation(playerPed->GetClump(), ASSOCGRP_STD,
									//ANIM_STD_IDLE, 8.0f);
									//phonePickAssoc->
									phonePickAssoc->SetFinishCallback(PhonePickOn, &phone);
									//bPickingUpPhone = true;
								
									
								}
								if(ImGui::Button("Phone on")) {
									wchar_t *msg = L"testtt";
									// gPhoneInfo.SetPhoneMessage_JustOnce(1, (wchar *)msg,0,0,0,0,0);
									// CPhone *phone = &gPhoneInfo.m_aPhones[1];
									//CPlayerInfo *playerInfo = playerPed->GetPlayerInfoForThisPlayerPed();
									//playerInfo->MakePlayerSafe(false);
									CAnimBlendAssociation *endAssoc = CAnimManager::BlendAnimation(
									    playerPed->GetClump(), ASSOCGRP_STD, ANIM_STD_PHONE_OUT, 8.0f);
									playerPed->SetPedState(PED_IDLE);
									//CAnimManager::BlendAnimation(playerPed->GetClump(), ASSOCGRP_STD, ANIM_STD_IDLE, 8.0f);
									/// phonePickAssoc->SetFinishCallback(PhonePickUpCB, &phone);
									// bPickingUpPhone = true;
								}

								 float x = 120;
								 float y = 10;
								 float l = SCREEN_SCALE_FROM_RIGHT(x);
								 float t = SCREEN_SCALE_Y(y);

								 float r = 40;
								 float b = 1;
								ImGui::SliderFloat("x1", &x, -1500.0, 1500.f);
								ImGui::SliderFloat("y1", &y, -1500.0, 1500.f);

								ImGui::SliderFloat("x2", &r, -1500.0, 1500.f);
								ImGui::SliderFloat("y2", &b, -1500.0, 1500.f);
								//CSprite2d::DrawRect(CRect(l, t, r, b),CRGBA(255, 0, 0, 255));
								ImGui::End();
							} else
								pad->SetEnableMouse();
